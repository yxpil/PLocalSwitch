...
> **PLocalSwitch** —— 本地部署的 LLM API 代理网关。**入站多协议**（OpenAI / Anthropic / Gemini 客户端可直接接入），**对外统一输出 OpenAI v1 契约**（`/v1/chat/completions` 等）并对各类上游做适配。内置 13 类厂商协议适配、双账本计费、节点质量评估、流式安全链路，并附赠一个 Tauri 桌面管理界面。
...
- **入站多协议 + 出站 OpenAI v1**：OpenAI / Anthropic / Gemini 客户端可直接接入；内部统一为 OpenAI 模型，对外输出标准 OpenAI v1 格式（`/v1/chat/completions`、`/v1/models`、`/v1/embeddings`），客户端零改动接入。
