//! Cohere v2 协议适配器 —— Cohere 提供 OpenAI 兼容端点（/v1/chat/completions），
//! 因此复用 OpenAIAdapter 的直通 + 规范化逻辑。
//! 注：配置 endpoint 请填 OpenAI 兼容 base（如 https://api.cohere.ai）。
use async_trait::async_trait;
use crate::error::AppResult;
use crate::models::{ChatCompletionRequest, ChatCompletionResponse, SseChunk};
use crate::router::CandidateNode;

pub struct CohereV2Adapter;

#[async_trait]
impl crate::backend_adapters::BackendAdapter for CohereV2Adapter {
    fn protocol(&self) -> crate::router::ProtocolKind { crate::router::ProtocolKind::CohereV2 }
