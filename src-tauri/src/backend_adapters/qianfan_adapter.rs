//! 百度千帆(Ernie) 协议适配器 —— 千帆提供 OpenAI 兼容端点（/v1/chat/completions），
//! 因此复用 OpenAIAdapter 的直通 + 规范化逻辑。
//! 注：配置 endpoint 请填 OpenAI 兼容 base（如 https://qianfan.baidubce.com/v2）。
...
#[async_trait]
impl crate::backend_adapters::BackendAdapter for QianfanAdapter {
    fn protocol(&self) -> crate::router::ProtocolKind { crate::router::ProtocolKind::Qianfan }
